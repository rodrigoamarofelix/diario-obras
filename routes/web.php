...
Route::middleware(['auth'])->group(function () {
    ...
    // ===== DIÁRIO DE OBRAS =====
    Route::middleware('auth')->prefix('diario-obras')->name('diario-obras.')->group(function () {
        // Dashboard do Diário de Obras
        Route::get('/', [App\Http\Controllers\DiarioObrasController::class, 'dashboard'])->name('dashboard');

        // Projetos
        Route::resource('projetos', App\Http\Controllers\ProjetoController::class);

        // Atividades de Obra
        Route::resource('atividades', App\Http\Controllers\AtividadeObraController::class);
        Route::get('atividades/projeto/{projeto}', [App\Http\Controllers\AtividadeObraController::class, 'porProjeto'])->name('atividades.por-projeto');

        // Equipe de Obra
        Route::resource('equipe', App\Http\Controllers\EquipeObraController::class);
        ...
        Route::get('materiais/projeto/{projeto}', [App\Http\Controllers\MaterialObraController::class, 'porProjeto'])->name('materiais.por-projeto');

        // Fotos de Obra
        Route::resource('fotos', App\Http\Controllers\FotoObraController::class);
        Route::get('fotos/projeto/{projeto}', [App\Http\Controllers\FotoObraController::class, 'porProjeto'])->name('fotos.por-projeto');
        Route::post('fotos/upload-multiple', [App\Http\Controllers\FotoObraController::class, 'uploadMultiple'])->name('fotos.upload-multiple');
        Route::patch('fotos/{id}/restore', [App\Http\Controllers\FotoObraController::class, 'restore'])->name('fotos.restore');
        Route::delete('fotos/{id}/force-delete', [App\Http\Controllers\FotoObraController::class, 'forceDelete'])->name('fotos.force-delete');
        Route::get('api/fotos/projeto/{projeto}/coordenadas', [App\Http\Controllers\FotoObraController::class, 'coordenadasProjeto'])->name('api.fotos.coordenadas');

        // ===== FUNCIONALIDADES AVANÇADAS =====

        // Relatórios Avançados
        Route::get('relatorios', function () {
            return view('diario-obras.relatorios.index');
        })->name('relatorios');

        Route::prefix('relatorios')->name('relatorios.')->group(function () {
            Route::get('projeto/{projeto}', [App\Http\Controllers\DiarioObrasController::class, 'relatorioProjeto'])->name('projeto');
            Route::get('produtividade', [App\Http\Controllers\DiarioObrasController::class, 'relatorioProdutividade'])->name('produtividade');
            Route::get('custos', [App\Http\Controllers\DiarioObrasController::class, 'relatorioCustos'])->name('custos');
        });

        // Exportação de Dados
        Route::prefix('exportar')->name('exportar.')->group(function () {
            Route::post('excel', [App\Http\Controllers\DiarioObrasController::class, 'exportarExcel'])->name('excel');
        });

        // Sistema de Alertas
        Route::get('alertas', [App\Http\Controllers\DiarioObrasController::class, 'alertas'])->name('alertas');

        // Configurações
        Route::get('configuracoes', [App\Http\Controllers\DiarioObrasController::class, 'configuracoes'])->name('configuracoes');
    });
    ...
    // ===== EMPRESAS (Fora do menu diário de obras) =====
    Route::middleware('auth')->group(function () {
        Route::resource('empresas', EmpresaController::class);
        Route::get('empresas/{empresa}/toggle-status', [EmpresaController::class, 'toggleStatus'])->name('empresas.toggle-status');
        Route::get('empresas/buscar-cep', [EmpresaController::class, 'buscarCep'])->name('empresas.buscar-cep');
        Route::get('empresas/validar-cnpj', [EmpresaController::class, 'validarCnpj'])->name('empresas.validar-cnpj');
        Route::patch('empresas/{id}/restore', [EmpresaController::class, 'restore'])->name('empresas.restore');
        Route::delete('empresas/{id}/force-delete', [EmpresaController::class, 'forceDelete'])->name('empresas.force-delete');
    });

    // ===== API PARA SINCRONIZAÇÃO OFFLINE =====
    Route::middleware('auth')->group(function () {
        Route::post('api/sync-photos', [App\Http\Controllers\FotoObraController::class, 'syncOfflinePhotos'])->name('api.sync-photos');
        Route::get('api/offline-data', [App\Http\Controllers\FotoObraController::class, 'getOfflineData'])->name('api.offline-data');
    });
