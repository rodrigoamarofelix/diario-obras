...
class EquipeObraController extends Controller
{
    ...
    {
        $equipe = EquipeObra::with(['projeto', 'atividade', 'funcionario', 'pessoa.funcao'])
            ->withTrashed()
            ->latest('data_trabalho')
            ->paginate(15);

        return view('diario-obras.equipe.index', compact('equipe'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'pessoas.*.horas_trabalhadas' => 'nullable|numeric|min:0|max:24',
    ...
    {
        $equipe->load(['projeto', 'atividade', 'funcionario']);
    ...
    public function update(Request $request, EquipeObra $equipe)
    {
        ...
        // Para edição, vamos assumir que é apenas uma pessoa por registro
        // Se precisar de múltiplas pessoas, seria necessário criar novos registros
        $pessoaId = $request->pessoas_selecionadas[0];
        $pessoa = Pessoa::with('funcao')->find($pessoaId);
